...
- [x] Logging phân tầng: `logs/auth.log` (login/LDAP) + `logs/app.log` (Django errors)
- [x] **Sidebar layout** scalable — `core/templates/core/base.html`
- [x] Dashboard: 4 stat cards + BHYT + sinh hoạt công dân + yêu cầu giấy tờ
- [x] Read-only models từ shared DB: `Student`, `Department`, `DegreeLevel`, `StudentStatus`, `HealthInsuranceCard`, `CivicActivity`
- [x] **Yêu cầu giấy xác nhận** — bảng `hub_confirmation_requests`, form tạo, danh sách theo dõi
...
Bảng của hub hiện tại: `hub_students`, `hub_confirmation_requests`.
...
students/models.py         ← read-only: Student, Department, DegreeLevel, StudentStatus,
                              HealthInsuranceCard, CivicActivity
config/settings.py         ← LDAP config, SESSION config, LOGGING, MIGRATION_MODULES
core/templates/core/
  base.html                ← layout gốc: sidebar + topbar (tất cả page extend từ đây)
  login.html               ← trang đăng nhập (standalone)
  home.html                ← dashboard chính
  confirmation_request_form.html ← form tạo yêu cầu giấy xác nhận
docs/schema.sql            ← SQL tạo bảng hub_*
docs/FEATURES.md           ← mô tả chi tiết các tính năng đã implement
docs/ECOSYSTEM.md          ← sơ đồ quan hệ với dashboard + WordPress
logs/auth.log              ← log login/logout/LDAP (rotate 5MB × 5)
...
| Tính năng đã implement chi tiết | `docs/FEATURES.md` |
